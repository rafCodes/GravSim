...
unsigned short colors[] = {red, dark_red, green, dark_green, blue, dark_blue, 
		yellow, cyan, magenta, gray, black, white};

// pixel buffer
extern volatile unsigned int * vga_pixel_ptr;
volatile unsigned int * vga_pixel_ptr = 0;

extern void *vga_pixel_virtual_base;
void *vga_pixel_virtual_base = 0;

// character buffer
extern volatile unsigned int * vga_char_ptr;
volatile unsigned int * vga_char_ptr = 0;

extern void *vga_char_virtual_base;
void *vga_char_virtual_base = 0;
...
void VGA_star(int x, int y, unsigned int pixel_color){
	...
	// make a + sign
	VGA_Hline(x-1, y, x+1, pixel_color);
	VGA_Vline(x, y-1, y+1, pixel_color);
}

/**
 * Make a star with diagonals and lines
*/
void VGA_super_star(int x, int y, unsigned int pixel_color){
	...
	VGA_line(x-i,y,x+i,y,pixel_color);
	VGA_line(x,y-i,x,y+i,pixel_color);
	VGA_line(x-j,y-j,x+j,y+j,pixel_color);
	VGA_line(x-j,y+j,x+j,y-j,pixel_color);
}
...
void VGA_warped_world(int x, int y, unsigned int pixel_color){
	VGA_Hline(x,x,y,pixel_color);
	VGA_Vline(x,y,y,pixel_color);
}